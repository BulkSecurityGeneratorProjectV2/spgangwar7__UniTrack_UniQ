    ...
    $('.filter10').DataTable( {
        initComplete: function () {
            this.api().columns(1).every( function () {
                var column = this;
                var select = $('<select class="custom-select tbl-select"><option value="">Subject Abbreviation Name</option></select>')
    ...
    $('.filter7').DataTable( {
        initComplete: function () {
            this.api().columns(3).every( function () {
                var column = this;
                var select = $('<select class="custom-select tbl-select"><option value="">Subject Name</option></select>')
                    .appendTo( $(column.header()).empty() )
                    .on( 'change', function () {
                        var val = $.fn.dataTable.util.escapeRegex(
                            $(this).val()
